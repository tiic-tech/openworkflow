...
`C002` is complete. Next selection should move to `C003`, which upgrades the
generated `/ow:proto` protocol so agents actually execute the preflight,
askUserQuestion, prompt-text, and image-generation flow encoded by the
artifacts.
