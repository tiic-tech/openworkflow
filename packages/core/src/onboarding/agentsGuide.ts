...
- Treat JSON \`ok:false\` as a failed command: OpenWorkflow exits nonzero but still writes the parseable JSON report to stdout for warnings, errors, and next_actions.
...
- Use \`openworkflow context --root . --json\` to materialize a compact startup packet for \`CURRENT_STATE.next_command\`; add \`--for /ow:<command>\`, \`--max-bytes <n>\`, or \`--mode full\` when you need a specific workflow command, tighter budget, or full managed audit source.
